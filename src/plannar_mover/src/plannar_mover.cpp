...
namespace gazebo
{
  class PlannarMover : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
    {
      ...
      this->model = _parent;
      GazeboRosPtr gazebo_ros_ = GazeboRosPtr ( new GazeboRos ( _parent, _sdf, "PlannarMover" ) );
      ...
      // Create a topic name
      std::string plannar_pos_topicName;
      gazebo_ros_->getParameter<std::string> ( plannar_pos_topicName, "commandTopic", "" );
      ...
      if (!ros::isInitialized())
      {
        int argc = 0;
        char **argv = NULL;
        ros::init(argc, argv, "plannar_node",
            ros::init_options::NoSigintHandler);
      }
         
      // Create our ROS node. This acts in a similar manner to
      // the Gazebo node
      this->rosNode.reset(new ros::NodeHandle("plannar_node"));
